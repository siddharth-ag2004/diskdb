...
### 5.1 Data Format and Identifiers
1.  Integer Data: We assume all data in the input CSV files (excluding the header row) consists of valid integers. This includes Node IDs, edge weights, and boolean attributes (0/1).
2.  Input Strictness: The input files are assumed to strictly adhere to the format specified in the problem statement (e.g., correct column ordering for Nodes and Edges).

### 5.2 Graph Semantics
3.  Undirected Bi-directionality: The input CSV for an undirected graph contains edges only once (e.g., 1,2). The system assumes implied bi-directionality. To support efficient querying, our internal storage handles this by indexing the edge in both directions (Forward and Reverse tables).
4.  Path Result Consistency: The resultant graph generated by the PATH command inherits the type of the source graph. If the source is Directed, the result is Directed; if Undirected, the result is Undirected.
5.  Attribute Consistency (ANY):
    *   ANY(N): We assume the attribute value of the Source Node dictates the required value for the entire path.
    *   ANY(E): We assume the path is valid if the specific attribute is consistent across all edges with either value 0 or value 1. The system checks both possibilities.

### 5.3 Naming and Namespace
6.  Graph Type Uniqueness: We assume the system does not contain a Directed Graph and an Undirected Graph with the exact same name simultaneously. Since commands like PATH do not specify the graph type (D or U), it would be impossible to disambiguate the target graph if both existed.
7.  Table/Graph Name Validity: Table and Graph names are assumed to be valid OS filenames and do not contain special characters.
8.  Namespace Conflicts: We assume that a Table and a Graph will not share the same name unless the command context explicitly disambiguates them.

### 5.4 System and Memory
...
Sidharth Agarwal (2022101062):
- Implemented external sorting mechanisms for disk-based graph data.
...
- Handled parsing and evaluation of WHERE clause conditions on nodes and edges, including ATTRIBUTE(N/E), ANY(N/E), equality checks, and uniform-attribute constraints.
- Implemented error handling for non-existent graphs, nodes, and invalid PATH queries as specified.
...
- Implemented LOAD GRAPH command with efficient page design and disk-based storage.
- Implemented EXPORT GRAPH functionality to regenerate node and edge CSV files in the required format and PRINT GRAPH command, including metadata printing and correct ordering of nodes and edges.
- Added binary search optimizations on pages for faster lookup.
- Testing using the project PDF specifications, covering edge cases such as empty WHERE clauses, conflicting conditions, invalid node IDs, and correctness of TRUE/FALSE outputs with path weights 
